...
#MODEL=Qwen/Qwen3-0.6B  # GRPO AGENT OK, GRPO ECHO OK with zero3_1node_2proc
MODEL=Qwen/Qwen3-1.7B  # GRPO SUDOKU default for q317b
...
# @@@ahoaho XXX
ENV_MODE=space  # this is default
#ENV_HOST=https://openenv-openspiel-env.hf.space  # this is default. unreachable from bv
#ENV_HOST=http://localhost:8000  # run the server as `uv run --project . server &`
ENV_HOST=http://localhost:8001  # run the server as `uvicorn textarena_env.server.app:app --reload --host 0.0.0.0 --port 8001 &`
#ENV_PORT=8001  # this is default
#ENV_IMAGE=textarena-env:latest  # this is default
...
ACCELERATE_CONFIG=accelerate_configs/zero3_1node_2proc.yaml  # SFT CUDA OOM for g338b, DPO OK for g4m, g4hm, g4ht dtype=bfloat16, DPO CUDA OOM for g418b dtype=bfloat16 per_device_train_batch_size=1 gradient_accumulation_steps=1, GRPO ECHO OK with zero3_1node_2proc
...
#OUTPUT_DIR="trainer_output/${MODEL##*/}${_DATASET_S}-grpo_catch-${START_TIME_STR}-${HOSTNAME_S}"  # NOTE neither timestamp nor hostname works with preemptable queue
OUTPUT_DIR="trainer_output/${MODEL##*/}${_DATASET_S}-grpo_catch"  # NOTE neither timestamp nor hostname works with preemptable queue
...
# See https://github.com/mtake/trl/blob/main/examples/grpo_catch/grpo_catch.py
# @@@ahoaho XXX
...
cmd="${ENV}accelerate launch --config_file ${ACCELERATE_CONFIG}${ACCELERATE_OPT} ${BASENAME}.py"
...
# @@@ahoaho 独自
# Environment
#cmd="$cmd --env_host https://openenv-openspiel-env.hf.space"
#cmd="$cmd --env_port 8001"
#cmd="$cmd --env_mode space"
#cmd="$cmd --env_image textarena-env:latest"

if [[ -n "${ENV_MODE}" ]]; then
    cmd="$cmd --env_mode ${ENV_MODE}"
fi
if [[ -n "${ENV_HOST}" ]]; then
    cmd="$cmd --env_host ${ENV_HOST}"
fi
if [[ -n "${ENV_PORT}" ]]; then
    cmd="$cmd --env_port ${ENV_PORT}"
fi
if [[ -n "${ENV_IMAGE}" ]]; then
    cmd="$cmd --env_image ${ENV_IMAGE}"
fi


# @@@ahoaho 独自
# Prompts
#cmd="$cmd --system_prompt_path sudoku_prompt.txt"
#cmd="$cmd --dataset_prompt 'Play Sudoku like an expert.'"
#cmd="$cmd --dataset_size 1000"

# @@@ahoaho 独自
# Game settings
#cmd="$cmd --max_turns 100"
#cmd="$cmd --difficulty easy"
#cmd="$cmd --api_delay 0.0"

# Sampling
cmd="$cmd --temperature 0.8"
cmd="$cmd --top_k 10"
#cmd="$cmd --top_p None"
...
cmd="$cmd --gradient_accumulation_steps 64"
cmd="$cmd --warmup_steps 20"
cmd="$cmd --per_device_train_batch_size 1"
cmd="$cmd --num_generations 8"
cmd="$cmd --num_train_epochs 1"
cmd="$cmd --max_completion_length 16384"
cmd="$cmd --resume_from_checkpoint True"

# Checkpoints
cmd="$cmd --save_steps 10"
#cmd="$cmd --save-total-limit None"
cmd="$cmd --output_dir ${OUTPUT_DIR}"

# Logging
cmd="$cmd --report_to trackio"  # default: none, choices: [none, all, trackio, wandb]
#cmd="$cmd --trackio_space_id Sudoku-GRPO"  # default: None (means local logging). The Hugging Face Space ID to use for live Trackio logging.
# * Trackio project initialized: grpo_sudoku_mtake
# * Trackio metrics logged to: /u/mtake/.cache/huggingface/trackio
# * View dashboard by running in your terminal: trackio show --project "grpo_sudoku_mtake"
# * or by running in Python: trackio.show(project="grpo_sudoku_mtake")
...
# LoRA / PEFT
cmd="$cmd --use_peft False"
cmd="$cmd --lora_r 16"
cmd="$cmd --lora_alpha 32"

# vLLM
cmd="$cmd --use_vllm True"
cmd="$cmd --vllm_mode colocate"
cmd="$cmd --vllm_server_base_url http://localhost:8000"
cmd="$cmd --vllm_gpu_memory_utilization 0.15"
